...
TouchManager::TouchManager(const DisplayConfig& config)
  : ts(config.pinTouchCs, 255), cfg(config) {}

void TouchManager::begin(SPIClass& spi) {
  ts.begin(spi);
  // Re-establish SPI pins in case ts.begin() reset them to defaults
  spi.begin(cfg.pinClk, cfg.pinMiso, cfg.pinMosi, -1);
...
bool TouchManager::poll() {
  ...
  // Poll the touchscreen chip directly via SPI to bypass the T_IRQ line completely
  if (ts.touched()) {
...
#else // TOUCH_SCREEN_ENABLED == 0 (Stub implementation)

TouchManager::TouchManager(const DisplayConfig& config)
  : cfg(config) {}

void TouchManager::begin(SPIClass& spi) {
  // Do nothing when touch screen is disabled
}

bool TouchManager::poll() {
  // Always return false (no touch events) when touch screen is disabled
  return false;
}
